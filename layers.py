...
class LinearSumma(Estimates):
    ...
    def compute_time_summa(self, flops, mem, comms, nv_sizes, comm_sizes, comm_types, comm_tops, verbose=False):
        ...
        for c, comm in enumerate(comms):
            ...
            t_for_nb_comms = self.get_time_comm(comm, nv, comm_size, comm_type, comm_top)  
            if verbose:
                print(self.name)
...
class Act(Estimates):
    def __init__(self, name, m, nv_domain ={'dim1': 1, 'dim2': 1}, system=None):
        """
...
class LogitsSumma(Estimates):
    ...
    def compute_time_summa(self, flops, mem, comms, nv_sizes, comm_sizes, comm_types, comm_tops):
        t_comp = self.get_time_flops(flops)
        t_mem  = self.get_time_mem(mem)
        t_compute = max(t_comp, t_mem)
        intensity = t_comp / t_mem
        ...
        for c, comm in enumerate(comms):
            ...
            nv_size = nv_sizes[c]
            # we are tracking total comm vols across n_b itrs
            t_for_nb_comms = self.get_time_comm(comm, nv_size, comm_size, comm_type, comm_top)  
            t_for_one_comm_set += t_for_nb_comms / self.n_b 
